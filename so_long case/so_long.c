...
void	init_map(t_env *env, char **map)
{
	...
	while (index.y_start + j < index.y_end)
	{
		...
		while (index.x_start + i < index.x_end )
		{
			...
			else if (map[index.y_start + j][index.x_start + i] == 'P')
				img_cpy(env->img.perso, &env->map.win_map[count*SIZE_IMG], env->mlx.win_x / SIZE_IMG, j);
				else if (map[index.y_start + j][index.x_start + i] == 'p')
				img_cpy(env->img.perso, &env->map.win_map[count*SIZE_IMG], env->mlx.win_x / SIZE_IMG, j);
...
int	handle_key(t_env *env)
{
	...
	print_nb_move(env);
	//print_map(map);
	if (env->map.nb_collected == env->map.nb_collectible && map[pos.new_y][pos.new_x] == 'p')
		{
			ft_putstr("\nvictoire!\n", 1);
			exit (0);
		}
